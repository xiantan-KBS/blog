...
Mock.mock('/api/blogtype', "get", {
    ...
    "data|10": [{
        "id|+1": 1,
        "name": "分类@id",
        "articleCount|0-100": 10,
        "order|+1": 1
    }]
});


///api/blog?page=1&limit=10&categoryid=-1
//分页获取博客
...
Mock.mock("/api/comment", "post", function(options) {
    ...
    // console.log(options, body, nickname, blogId, content);
...
Mock.mock(/^\/api\/blog\/.+$/, "get", function(options) {
    // console.log(options);
    const id = options.url.substr(10);
    data1.data.id = id;
...
Mock.mock(/^\/api\/comment(\?.+)?$/, "get", function(options) {
    const query = qs.parse(options.url); //{/api/comment?blogid: "Fce4D78D-BD2b-4EFD-d8eB-54f2651d01c7",limit: "10",page: "1"}
    return Mock.mock({
        code: 0,
        msg: "",
        data: {
